...
const ResetPassword = ({token} : {token : string}) => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!password || !confirmPassword) {
      ...
      try {
        ...
        if (response) {
          toast.success("Password has been resat successfully, You can login now")
          router.push("/login")
        } else {
          toast.error("This link is not valid anymore")
        }
      } catch (err) {
  ...
  return (
    <div>
        <p className="text-3xl font-bold mb-2 text-center"> Reset Password </p>
        <p className="opacity-70 font-medium mb-6 text-center"> Add your new password </p>
        {/* Password */}
        ...
        {/* Button */}
        <div onClick={e => handleRegister(e)}><NormalButton label="Reset" styles="w-full" loading={loading} /></div>
    </div>
  )
}
